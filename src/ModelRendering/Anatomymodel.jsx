...
import { fetchUnitData } from '../Database/databaseUtils.js';
...
function Anatomymodel (props) {
  ...
  useEffect(() => {
    if (!selectedGender || !bodySize) return;
  
    fetchUnitData('Unit 1: Organs, Systems, and the Organization of the Body')
      .then((unitData) => {
        const { modelfiles } = unitData;
        const models = selectedGender === 'female' ? modelfiles.femaleModels : modelfiles.maleModels;
  ...
  const bodyModelPath = modelPaths[`body${bodySize}`];
  ...
  const { nodes, materials } = useGLTF(bodyModelPath);
